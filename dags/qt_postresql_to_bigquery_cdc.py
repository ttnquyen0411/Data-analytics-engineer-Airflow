...
def extract_postgresql_to_gcs_func(ds, **kwargs):
    ...
    sql = f"""
    SELECT * FROM products WHERE updated_at >= '{start_time_str} UTC' AND updated_at < '{end_time_str} UTC'
    """
    print(sql)
    ...
    for row in postgres_cursor:
        ...
        if count >= 1000:
            ...
            upload_filename = f"data/products/{upload_file_prefix}/{file_no}.parquet"  # the name of blob
    ...
    # schema_upload_filename = f"data/products/{upload_file_prefix}/schema.json"  # the name of blob
...
def load_extracted_file_to_bigquery_func(ds, **kwargs):
    ...
    table_id = f"bigquery_change_data_capture_example.products_delta"
    dag_run = kwargs["dag_run"]
    execution_date = dag_run.execution_date
    end_date = execution_date.replace(minute=0, second=0, microsecond=0)
    start_time = end_date - datetime.timedelta(hours=1)
    ...
    job = bigquery_client.load_table_from_uri(
        source_uris=f"gs://{bucket_name}/data/products/{upload_file_prefix}/*.parquet",
...
with DAG(
    dag_id="qt_postgresql_to_bigquery_cdc",
