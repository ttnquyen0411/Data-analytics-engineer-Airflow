...
def extract_postgresql_to_gcs_func(ds, **kwargs):
    ...
    for row in postgres_cursor:
        ...
        if count >= 1000:
            ...
            upload_filename = f"data/order_items/full/{file_no}.parquet"  # the name of blob
...
def load_extracted_file_to_bigquery_func(ds, **kwargs):
    ...
    table_id = f"bigquery_change_data_capture_example.order_items_main"
    bucket_name = "data-analytics-engineer-example-quyentran"

    bigquery_client = bigquery.Client(credentials=credentials)
    job = bigquery_client.load_table_from_uri(
        source_uris=f"gs://{bucket_name}/data/order_items/full/*.parquet",
...
with DAG(
    dag_id="qt_from_data_warehouse_to_dimension_type_2_order_items",
